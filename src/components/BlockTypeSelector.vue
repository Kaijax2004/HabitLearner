<template>
  <transition name="block-selector">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm" @click="close">
      <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto" @click.stop>
        <!-- 头部 -->
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">选择内容类型</h3>
          <button @click="close" class="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        <div class="mb-6">
          <div class="relative">
            <input 
              ...
              type="text" 
              placeholder="搜索内容类型..."
              class="w-full px-4 py-3 pl-10 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
            />
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button 
            ...
            class="p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all duration-200 flex items-center gap-4 group hover:shadow-md transform hover:-translate-y-0.5"
          >
            <div 
              class="w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200"
              :class="blockType.colorClass"
            >
              <component :is="blockType.icon" class="w-6 h-6" :class="blockType.iconClass" />
            </div>
            <div class="text-left flex-1">
              ...
              <div v-if="blockType.shortcut" class="text-xs text-gray-400 dark:text-gray-500 mt-1">
                快捷键: <kbd class="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs">{{ blockType.shortcut }}</kbd>
              </div>
            </div>
          </button>
        </div>
        ...
        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 text-center">
          <p class="text-xs text-gray-500 dark:text-gray-400">
            按 <kbd class="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs">ESC</kbd> 关闭，或按 <kbd class="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs">/</kbd> 快速打开
          </p>
        </div>
      </div>
    </div>
...
const blockTypes = [
  ...
  },
  {
    ...
    colorClass: 'bg-blue-100 dark:bg-blue-900/30 group-hover:bg-blue-200 dark:group-hover:bg-blue-800/50',
