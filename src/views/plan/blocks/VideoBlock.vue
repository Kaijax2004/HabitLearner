<template>
  <div class="rounded-2xl border border-zinc-200 bg-white px-4 py-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
    <div v-if="model.url" class="space-y-3">
      <video :src="model.url" controls class="w-full rounded-2xl bg-zinc-100 dark:bg-zinc-900"></video>
      <div class="flex flex-wrap gap-2">
        <button type="button" class="rounded-full bg-zinc-950 px-3 py-1.5 text-xs text-white dark:bg-white dark:text-zinc-950" @click="triggerFileInput">替换视频</button>
        <button type="button" class="rounded-full border border-zinc-200 px-3 py-1.5 text-xs text-zinc-600 dark:border-zinc-700 dark:text-zinc-300" @click="clearVideo">移除</button>
      </div>
    ...
    <div
      ...
      @drop.prevent="onDrop"
    >
      <div class="text-sm text-zinc-600 dark:text-zinc-300">点击上传视频或拖拽到此处</div>
...
const clearVideo = () => emit('update:modelValue', { url: '', title: '', mimeType: '' })
