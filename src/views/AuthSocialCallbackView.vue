  ...
  <div class="social-callback-page">
    <div class="social-callback-card">
      ...
      <form v-if="needsProfileCompletion" class="social-complete-form" @submit.prevent="submitProfile">
        ...
        <p class="social-note">第三方快捷注册暂不绑定邮箱，进入工作台后可在“我的 / 账户安全”中绑定真实邮箱。</p>
...
onMounted(async () => {
  ...
  const ticket = params.get('profileTicket')
  ...
  } else if (!ticket && !authStore.isAuthenticated) {
    hasError.value = true
  ...
  if (complete === '0' || profileTicket.value || !authStore.user?.profileComplete) {
