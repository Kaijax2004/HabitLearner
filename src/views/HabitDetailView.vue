...
const todayProgress = computed(() => {
  ...
  const result = habit.value.checkIns.find(checkIn => {
    ...
    if (checkIn.checkInTime) {
      ...
      effectiveDate = new Date(checkIn.checkInTime).toISOString().split('T')[0]
    } else {
...
const weeklyData = computed(() => {
  ...
  const createDateString = createDate.toISOString().split('T')[0]
...
const monthlyData = computed(() => {
  ...
  for (let d = new Date(firstDay); d <= lastDay; d.setDate(d.getDate() + 1)) {
    const dateString = d.toISOString().split('T')[0] // 格式: YYYY-MM-DD
...
const recentRecords = computed(() => {
  ...
  sortedCheckIns.forEach(dayCheckIn => {
    ...
    if (dayCheckIn.checkInTime) {
      ...
      effectiveDate = localTime.toISOString().split('T')[0]
    } else {
      // 如果没有checkInTime，使用date字段
